...
if (apiKey !== '') {
    import('https://openfpcdn.io/fingerprintjs/v4')
        ...
        .then(result => {
            ...
            if (userID) {
                fetch(`https://sr.adrig.co.in/chatlaps/pageview?id=${apiKey}`)
                    ...
                    .then(data => {
                        if (data) {
                            ...
                            if (logoPosition === 'left') {
                                rootDiv.style.left = '1rem';
                                rootDiv.style.right = 'unset';
                            } else {
                            ...
                            const applyMobileStyles = () => {
                                if (window.matchMedia('(max-width: 768px)').matches) {
                                    ...
                                    rootDiv.style.width = '448px';
                                    rootDiv.style.height = logoBottomPosition > 50 ? `${80}dvh` : '85dvh';
                                    rootDiv.style.bottom = `${logoBottomPosition + 70}px`;
                                    rootDiv.style.right = '1rem';
                                    rootDiv.style.borderRadius = '0.75rem';
                                }
                            };
                            ...
                            // iframe.src = `http://localhost:5173/${apiKey}/${userID}`; https://arivubot-widget.vercel.app
                            iframe.src = `https://arivubot-widget.vercel.app?apiKey=${apiKey}&userId=${userID}&bgColor=${encodeURIComponent(
                                bgColor
                            )}&fontSize=${encodeURIComponent(
                                fontSize
                            )}&fontColor=${encodeURIComponent(
                                fontColor
                            )}&fontStyle=${encodeURIComponent(
                                fontStyle
                            )}&headerAlign=${encodeURIComponent(
                                headerAlign
                            )}&innerButtonColor=${encodeURIComponent(
                                innerButtonColor
                            )}&outerButtonColor=${encodeURIComponent(
                                outerButtonColor
                            )}&userChatBg=${encodeURIComponent(
                                userChatBg
                            )}&botChatBg=${encodeURIComponent(
                                botChatBg
                            )}&imgUrl=${encodeURIComponent(
                                imgUrl
                            )}&logoPosition=${encodeURIComponent(
                                logoPosition
                            )}&logoBottomPosition=${encodeURIComponent(
                                logoBottomPosition
                            ...
                            chatButton.style.padding = 'unset';
                            chatButton.style.position = 'fixed';
                            chatButton.style.border = '0px';
                            chatButton.style.bottom = logoBottomPosition ? `${logoBottomPosition}px` : '16px';
                            chatButton.style.width = '60px';
                            chatButton.style.height = '60px';
                            chatButton.style.borderRadius = '29.5px';
                            chatButton.style.backgroundColor = outerButtonColor ? outerButtonColor : 'rgb(0, 0, 0)';
...
<path  d="M15.2347 2.53476C14.2201 2.1881 13.132 2 12 2C6.47715 2 2 6.47715 2 12C2 13.5997 2.37562 15.1116 3.04346 16.4525C3.22094 16.8088 3.28001 17.2161 3.17712 17.6006L2.58151 19.8267C2.32295 20.793 3.20701 21.677 4.17335 21.4185L6.39938 20.8229C6.78393 20.72 7.19121 20.7791 7.54753 20.9565C8.88836 21.6244 10.4003 22 12 22C17.5228 22 22 17.5228 22 12C22 10.868 21.8119 9.77987 21.4652 8.76526C20.7572 9.22981 19.9101 9.5 19 9.5C16.5147 9.5 14.5 7.48528 14.5 5C14.5 4.08987 14.7702 3.24284 15.2347 2.53476Z" fill="#fff"/>
                            ...
                            chatButton.innerHTML = imgUrl
                                ? `<img src="${imgUrl}" alt="Chat Icon" style="padding: unset; margin: unset; height:100%; width:100%; border-radius: 50%; display: block;">`
                                : svg;
